...
class Contact {	
	...
	public function listContact(){
		...
		$params = array();
		if($_SESSION["role"] == 'sales') { 
			$sqlWhere = " WHERE c.sales_rep = ?";
			$params[] = $_SESSION["userid"]; // Collect parameters for the WHERE clause
		}   
		
		$sqlQuery = "SELECT c.id, c.contact_first, c.company,  c.budget, u.name 
		FROM ".$this->contactTable." c
		LEFT JOIN ".$this->userTable." u ON c.sales_rep = u.id $sqlWhere";
		
		if(!empty($_POST["order"])){
			// Use $_POST['columns'][$_POST['order'][0]['column']]['data'] to get the correct column name
			$sqlQuery .= ' ORDER BY '.$_POST['columns'][$_POST['order'][0]['column']]['data'].' '.$_POST['order'][0]['dir'].' ';
		} else {
			$sqlQuery .= ' ORDER BY id ASC ';
		}
		...
		$stmtTotal = $this->conn->prepare("SELECT COUNT(*) as total FROM ".$this->contactTable.$sqlWhere);
		if($_SESSION["role"] == 'sales') {
			$stmtTotal->bind_param("i", $_SESSION["userid"]); // Bind the session user ID parameter
		}
		$stmtTotal->execute();
		$allResult = $stmtTotal->get_result();
		$allRecords = $allResult->fetch_assoc()["total"];
